 ...
 * 实名认证信息（跨服务只读投影）。
 *
 * user_identity 归 user 服务写，hotel 服务只在这一处读它：保存酒店订单前
 * 确认下单人已实名且**审核通过**，并把姓名与证件号抄进订单。两个服务连的是同一个库，
 * 和 attraction 的 RealNameDao 读同一张表是同一类取舍 ——
 * 多一张表的耦合，省掉一次服务间调用。
 *
 * 与 com.zhiyouxing.attraction.entity.vo.RealNameInfo、travel 的同名类同源，
 * 是同一套字段的第三份拷贝（为什么不上提到 common，见各个 RealNameDao 的类注释）。
 */
